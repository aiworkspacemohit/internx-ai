...
import { Building2, CheckCircle2, XCircle, Globe, MapPin } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      <div className="glass-card p-6 rounded-3xl border border-slate-700/80">
        <h1 className="text-2xl font-extrabold text-white">Company Registration Approvals</h1>
        <p className="text-xs text-slate-400">Review corporate credentials and authorize recruiters to post internships</p>
      </div>

      {loading ? (
        <div className="text-center py-12 text-indigo-400">Loading company requests...</div>
      ) : companies.length === 0 ? (
            ...
            <div key={c.id} className="glass-card p-6 rounded-2xl border border-slate-700/80 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start space-x-3">
                  <div className="w-12 h-12 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-amber-400 text-lg">
                    {c.company_name?.charAt(0)}
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-white">{c.company_name}</h3>
                    <p className="text-xs text-amber-400 font-semibold">Verification: {c.verification_status}</p>
